...
**Option D: Use Perplexity Sonar Pro (PREFERRED for deep research)**
- Grounded web research with inline citations and source URLs
- Uses `sonar-pro` model: 200K context, deeper search, 2-3x more sources than alternatives
- Requires `PERPLEXITY_API_KEY` in `.env`
- Call via Bash with curl:
  ```bash
  ...
  ```
- Response includes `citations` array with source URLs and `choices[0].message.content` with grounded answer
- Best for: complex multi-source queries, triangulation, trend analysis, market research
- Use alongside WebSearch for maximum source diversity (Perplexity searches different indices)
- Cost: ~$0.01-0.03 per query. Use for key research questions, not every sub-query.

**NEVER mix parameter styles** - this causes "Invalid tool parameters" errors.

**Step 1b: Run last30days for community signal (when relevant)**
