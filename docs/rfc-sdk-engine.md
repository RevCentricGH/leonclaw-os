# RFC: SDK Engine — Direct API Backend for LeonClaw
...
giving LeonClaw direct control over the agentic loop, tool execution, token accounting,
...
### 3. Tighter integration with LeonClaw internals

The CLI subprocess runs in its own process with its own environment. Passing secrets
requires `env` injection (`sdkEnv` in `agent.ts:117-123`). Reading LeonClaw's SQLite
...
- **Custom tool schemas**: define LeonClaw-specific tools (e.g., `MemorySearch`,
...
1. **Session management is opaque**: The CLI manages session files internally.
   LeonClaw only stores the `session_id` string in SQLite (`sessions` table)
   and passes it back via `resume`.

2. **Tool execution is invisible**: LeonClaw sees `tool_progress` events with
...
4. **System prompt is implicit**: CLAUDE.md is loaded by the CLI from `cwd`.
   LeonClaw's `agentSystemPrompt` is prepended to the user message, not
...
The CLI engine manages sessions opaquely — LeonClaw only stores the session ID.
...
| 6 | **In-process tool access** | Tools can call `db.ts` functions directly, read LeonClaw state |
...
Since LeonClaw is a personal bot on a trusted machine with `ALLOWED_CHAT_ID` filtering,
...
Future option: support a subset — e.g., MCP servers defined in a LeonClaw config file
...
LeonClaw already disables the CLI's permission system (`permissionMode: 'bypassPermissions'`).
The SdkEngine would run tools with the same user privileges. Bash commands run as the
LeonClaw process user. File operations have full filesystem access.
...
Run the `claude` CLI in server mode, call it via HTTP from LeonClaw.
...
A typical LeonClaw turn with Opus (200k context, 2k output) costs ~$3.15.
